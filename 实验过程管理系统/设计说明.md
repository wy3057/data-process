...
**短句**：未登录不得访问业务接口。  
**展开说明**：
- 任何 `/plans`、`/tasks`、`/records`、`/progress`、`/reports` 的操作都应依赖会话登录态。  
- 页面路由与 API 路由都要做同等保护，不能只保护其中一层。  

### 19.2 用户隔离必须体现在“查询条件”上

**短句**：用户隔离基于 `current_user.id`。  
**展开说明**：
- 隔离不是“前端不显示别人数据”这么简单，而是后端查询本身就不返回他人数据。  
- `get_user_plan/get_user_task` 这类函数就是隔离落地点：通过“资源ID + 当前用户ID”联合校验实现防越权。  

### 19.3 统一错误响应是安全与可维护性的共同要求

**短句**：统一错误避免暴露内部实现。  
**展开说明**：
- API 统一用 `api_error` 返回结构化错误，避免不同模块各说各话。  
- 对外不返回 SQL 语句、路径、堆栈。  
- 日志内部保留足够上下文（模块、资源ID、trace_id）供排障。  

### 19.4 SECRET_KEY 与 HTTPS 的真实意义

**短句**：生产启用 HTTPS 与强随机 `SECRET_KEY`。  
**展开说明**：
- `SECRET_KEY` 弱或泄露，会导致会话可伪造。  
- 没有 HTTPS，即使密钥强，传输过程中的 Cookie 仍可被窃听。  
- 两者必须同时满足：
  1) 强密钥保护签名；
  2) HTTPS 保护链路；
  3) Cookie 安全属性减少浏览器侧风险。  


---

## 20. 算法实现细节（与 Flask 模块函数对应）

### 20.1 认证算法（`register` / `login` + `User.set_password/check_password`）

**目标**：保证口令安全存储与会话登录。  
**输入**：用户名、密码。  
**输出**：登录态（session）或错误响应。

**实现步骤**：
1. 注册时检查用户名是否重复。
2. 调用 `set_password` 进行哈希存储。
3. 登录时用 `check_password` 验证。
4. 成功后 `login_user` 建立会话。

**算法要点**：
- 哈希与验证由 `werkzeug.security` 提供，避免自行实现密码算法细节。

### 20.2 权限隔离算法（`get_user_plan` / `get_user_task`）

**目标**：防越权访问他人资源。  
**输入**：资源 ID、`current_user.id`。  
**输出**：资源对象或 `None`。

**实现思路**：
- 查询条件必须同时包含 `id` 与 `owner_id=current_user.id`。

**伪代码**：
```text
get_user_task(task_id):
  return query(Task).filter(Task.id==task_id, Task.owner_id==current_user.id).first()
```

### 20.3 参数解析算法（`parse_int` / `parse_date`）

**目标**：把不可靠输入转为类型安全值。  
**步骤**：
1. 读取原始字符串。
2. 尝试转换（`int()` / `datetime.strptime`）。
3. 失败时抛出验证错误并走 `api_error` 统一返回。

**收益**：
- 业务函数可假定输入类型正确，减少重复校验。

### 20.4 任务更新算法（`update_task`）

**目标**：支持 PATCH 局部更新且保持数据合法。  
**步骤**：
1. 通过 `get_user_task` 获取且校验归属。
2. 读取 payload 中存在的字段。
3. 只更新允许变更字段。
4. 提交事务并返回序列化结果。
